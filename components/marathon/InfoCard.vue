...
const totalPrice = computed(() => {
  if (price.value) {
    return (
      (price.value?.hotel_number?.price || 0) +
      (price.value.visa?.service?.price || 0) +
      (price.value.visa?.consular_fees?.price || 0)
    );
  }
  return 0;
});
...
<template>
  <div class="info-card sticky top-5 bg-white">
    ...
    <div
      v-if="price"
      class="border-neutral-90 border-t pt-8"
    >
      <p class="mb-5 text-lg font-semibold">{{ t('cost-order') }}</p>

      <ul class="flex flex-col gap-5 border-b pb-5">
        <li
          v-if="price.hotel_number"
          class="flex justify-between text-base"
        >
          <b class="font-medium">{{ get(price.hotel_number?.hotel, 'name_for_site') }}</b>
          <span>{{ price.hotel_number?.currency?.symbol }} {{ price.hotel_number?.price }}</span>
        </li>
        <li
          v-if="price.visa"
          class="flex justify-between text-base"
        >
          <b class="font-medium">{{ t('visa') }} {{ get(price.visa, 'name') }}</b>
          <span>{{ price.visa?.service?.currency?.symbol }} {{ price.visa?.service?.price }}</span>
        </li>
        <li
          v-if="price.visa"
          class="flex justify-between text-base"
        >
          <b class="font-medium">{{ t('cons-sbor') }} {{ get(price.visa, 'name') }}</b>
          <span>{{ price.visa?.consular_fees?.currency?.symbol }} {{ price.visa?.consular_fees?.price }}</span>
        </li>
      </ul>

      <p class="mt-5 mb-8 flex justify-between text-lg font-semibold">
        {{ t('itogo') }}
